...
// 其他杂项设置
live2d_settings['l2dVersion']           = '0.0.1';        // 当前版本
live2d_settings['l2dVerDate']           = '2026.1.9'; // 版本更新日期
live2d_settings['homePageUrl']          = 'https://xytengc.github.io';       // 主页地址，可选 'auto'(自动), '{URL 网址}'
...
function initModel(waifuPath, type) {
    ...
    $('.waifu-tool .fui-home').click(function (){
        window.open(live2d_settings.homePageUrl);
    });
...
function loadTipsMessage(result) {
    	    ...
    	    default:
                ...
                $.ajax({
                    ...
                    error: function(xhr, status, error) {
                        console.warn('API请求失败，使用本地消息:', error);
                        // API失败时使用本地消息
                        fallbackToLocalMessage();
                    }
                });
    	}
    }
    ...
    setTimeout(function() {
        // 重新绑定聊天按钮，使用本地版本
        $('.waifu-tool .fui-chat').off('click').on('click', function() {
            console.log('聊天按钮点击 - 修复版');
            ...
            showMessage(msg, 4000, true);
        });
        
        // 重新绑定其他按钮（确保可用）
        $('.waifu-tool .fui-eye').off('click').on('click', loadOtherModel);
        $('.waifu-tool .fui-user').off('click').on('click', loadRandTextures);

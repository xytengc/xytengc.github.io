...
# 实习总结
实习内容：负责项目基于FPGA远程IO控制系统的开发。  
项目背景：在原有PLC数据采集设备基础上进行业务拓展，希望能针对不同传感器采集后的ADC数据用不同的滤波算法进行滤波，
所以需要相对现有设备更高的数据传输速度以及更高采样率的ADC，因此提出使用FPGA进行拓展实验。
项目介绍：首先是经过资源评估选择了安路的EF2系列FPGA测试板，根据产品要求以及各芯片手册设计产品仕样书，同时也参考了EtherCAT网络拓扑。根据产品仕样书，进行模块设计与验证到综合功能验证再到模拟测试，最后基于各芯片手册按要求设计电路。  
本产品开发内容涉及  
    基于IP硬核AST的LVDS板间通信（375M）  
    IIC读写EEPROM完成设备初始化(500K)  
    MCU基于UART与FPGA通信(460K)  
    基于HCT693x的ADC采集控制(500K~5M)  

实现功能：由MCU向FPGA主设备发送报文，对各个从设备进行访问与操作以达到控制的目的。在上述频率条件下，可以达到1ms内完成一次MCU-主机-从机的报文回环，其中主从板间通信AST为15us。  
重点难点：合理利用FPGA芯片的资源、不同模块间的跨时钟域处理、系统稳定性等等。  

具体设计详情：首先根据FPGA测试版对DEMO进行复现验证开发板正常，并熟悉软件使用。之后根据设计框图，对各个通信模块进行设计验证  
1、AST通讯模块（通用）：此模块用于板间通信，此模块直接使用的EF2芯片自带的硬核进行设计，每块芯片带两个硬核，为每个硬核设计发送模块和接收模块，采用LVDS接口协议进行传输。对于一个从机设备而言有需要向前收发和向后收发。向前接收模块接收前一个设备的报文进行解帧并发给APP层，向后发送模块输入APP层解析完的报文内容进行组帧并输出给AST IP核进行发送，向后接收模块接收AST收到的报文进行解帧，向前发送将AST解帧完的报文直接组帧发送。对主机而言只需向后发送与向后接收，向前通过UART传输给MCU。 具体如何设计的：驱动模块使用的是硬核，只需根据芯片手册配置时钟即可，数据收发控制模块先使用FIFO进行缓存，然后进行解码报文内容，传递给APP层分析。  
2、IIC读写EEPROM模块（从机）：此模块用于从机的上电初始化，用于还原设备上一次掉电的状态；在正常工作情况下，可以通过指令对初始值进行修改，包括设备ID、设备初值。 具体设计：首先大体分为驱动模块设计和控制模块设计，驱动模块根据芯片的时序要求设计读写芯片状态机，可分为空闲态、写地址控制、等待响应1、发送高8位存储寄存器地址、等待响应2、发送低8位存储寄存器地址、等待响应3、写数据、应答4、STOP、写读控制信号、等待读数据、非响应信号。
3、基于UART与MCU通讯模块（主机）：此模块类似与从机的向前收发，用于发送指令与接收反馈指令。  Uart具体设计：大体分为一个包含收发驱动模块的顶层控制模块，驱动根据SCL、SDA、Baud、CHeck等信息配置收发数据  
4、ADC采集模块：用于采集数据与数据处理。具体设计：大体分为驱动和控制模块、驱动设计：首先根据读写数据时序、和寄存器配置信息规划一次完整写数据、读数据的时序状态图，再根据输入输出区分为用户控制接口和物理驱动接口、用户控制接口连接控制模块、通过两边握手信号和忙信号交互运行。控制模块则根据寄存器配置顺序设计状态机，先进行全局复位、软复位、读写寄存器状态、重新配置寄存器、等待驱动端接收数据，状态机二段通过忙信号和上一次寄存器写成功验证进行状态转换、状态机三段通过二段的状态进行用户数据寄存输出。  
本次实习主要有三点收获：  
第一点是关于FPGA芯片资源的合理利用，无论任何项目的开展或者算法的硬件部署，都要先考虑硬件是否能够支持，尤其当FPGA硬件资源有限时该如何合理分配与高效利用。由于是第一次独立做项目经验不足且调研不够，导致实践过程中曾遇到多次资源不足的情况。在AST通信过程中需要对接受与发送的数据进行处理，对于占用资源较高的又需要频繁操作的数据需要酌情考虑是使用FPGA的LUT资源进行缓存还是使用BRAM；在ADC采集数据时，需要对ADC数据进行还原并保存用于后续的算法，后续算法即会用到了FPGA内部的DSP资源，由于所选FPGA芯片DSP资源有限，该如何使用DSP能做到使用率最大化并不超出最大DSP资源以节约LUT资源。  
第二点是关于**不同模块间的跨时钟域处理**，这是整个项目会持续遇到的问题。由于涉及到不同芯片之间的协同，不同接口协议的时钟频率是不一样的，必然会涉及跨时钟域处理。比如IIC读写EEPROM的部分一般情况是500K的时钟频率，EEPROM会在上电时进行初始化并且在修改初始化数据时进行写数据操作，读数据初始化是慢时钟到快时钟的操作，频率差几十倍，只需进行正常采样即可，而写数据是快时钟到慢时钟的操作，所以数据在进入IIC模块后需要保持一段时间再进行采样；在SPI进行ADC采样时使用400K左右的数据采样率，则对于FPGA则可以直接使用异步双端口RAM进行缓存；在UART与上位机进行通信时时钟在460K左右，则对于上位机通过UART发送的指令可以用异步FIFO进行存取。**如何解决**：对于输入的串行数据多数是慢时钟域到快时钟域进行打拍处理即可，对于LVDS等时钟较快的数据，并行数据通过异步FIFO进行缓存，并对使能信号进行展宽打一拍处理做到与FIFO输出数据同步；对于慢时钟使能信号进行打拍同步即可。  
第三点是如何保证**系统稳定性**。对于一个工控系统而言，最重要的就是在保证业务完整的情况下确保系统稳定运行，包括长时间运行下信号的完整性，对于不规范指令的容错等等。每次开机上电会工作长达几个月甚至不断电，因此需要考虑在长时间工作情况遇到数据报错如何解决；对于本项目遇到的传输过程丢报文、传输报文出错、乱码等问题，首先一定要排除的是硬件问题和环境条件，比如电源不稳定、LVDS传输线松动、外界电磁干扰对LVDS传输线的影响等等，在排除这些问题后再者考虑程序是否完备。如何解决：  

以上是对实习期间的FPGA工作的一些收获与总结，相比课题实践，在公司做产品是有很大的区别，不仅需要考虑功能的实现，更需要考虑产品的实用性，面对不同客户不同应用场景对产品有不同的要求。同时实习实践过程的有些技能是需要多加强的，比如使用示波器对信号的观察与在使用ChipWatcher抓取信号是不一样的，FPGA到底是硬件的开发，所以在进行实验时需要多使用示波器数字信号发生器进行信号分析。经过三个月的实习实践，进一步加强了我对FPGA的理解与开发能力，对后面课题的研究实践提供很好的经验。  
